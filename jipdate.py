...
def parse_status_file(jira, filename, issues):
    ...
    for line in status:
        ...
        if match:
            ...
            # We had fetched already, but issue is not found
            # or we haven't queried Jira yet, at all
...
def main(argv):
    ...
    # At this point, we might have fetches some issues already, if so, let's use them to
    # avoid fetching them again later.
    try:
        issues
    except NameError:
